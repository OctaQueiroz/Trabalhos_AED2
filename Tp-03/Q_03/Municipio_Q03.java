...
class Municipio_Q03{
  ...
  public static void main (String[]args)throws Exception{
    ...
    Municipio[] municipioPilha = new Municipio[5571];
    entrada = MyIO.readLine();
    for(i = 0; i < 5571; i++){
      municipioPilha[i] = new Municipio();
    }
    
    i = 0;
    do{
      novo.ler(novo, Integer.valueOf(entrada));
      municipioPilha[i] = novo.clone();
      entrada = MyIO.readLine();
      i++;
      cont2++;
    }while(entrada.equals("0") == false);   

    PilhaMunicipio pilha = new  PilhaMunicipio(500);
    ...
    do{
      ...
      if(temporaria[0].equals("I") == true){
        ...
        novo = pilha.remover();
        MyIO.print("(R) ");
        MyIO.println(novo.getNome());
      }
      cont++;
    }while(cont<=aux);
    for(i = 0; i < cont2; i++){
      municipioPilha[i].imprimir();
    }
    pilha.mostrar();
  }
}
...
class PilhaMunicipio{
  private Municipio[] array;
  private int topo;

  public PilhaMunicipio(int tamanho){
