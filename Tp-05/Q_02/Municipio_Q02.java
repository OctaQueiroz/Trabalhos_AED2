...
class Municipio_Q02{
  ...
  public static void main (String[]args)throws Exception{
    ...
    entrada = MyIO.readLine();
    do{
      aux = buscaBin(lista,Integer.valueOf(entrada), cont);
      if(aux == 1){
        MyIO.println("SIM");
      }
      else{
    ...
    tempo = tempo2 - tempo;
    Arq.openWrite("529934_sequencial.txt");
    Arq.print("529934\t"+cont2+"\t"+tempo);
    Arq.close();
  }

  public static int buscaBin(ListaMunicipio lista, int id, int tam){
    Municipio aux;     
    Municipio aux2;
    int n = tam;
    int fim = tam;     
    ...
    for(int i = 0; i < n-1; i++){   
      for(int j = i+1 ; j < n; j++){
        aux = lista.retornaItem(i);
        aux2 = lista.retornaItem(j);
