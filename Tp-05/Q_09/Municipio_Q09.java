...
class Municipio_Q09{
  ...
  public static void main (String[]args)throws Exception{
    ...
    lista.countingsort(cont2);
    lista.mostrar();
    tempo2 = System.currentTimeMillis();
    tempo = tempo2 - tempo;
    Arq.openWrite("529934_quicksort.txt");
...
class ListaMunicipio{
  ...
  public int[] ordenaID(int[] cont2){
    for(int i = 0; i< n-1; i++){
      if(vet[i].getNumeroFuncionarios() == vet[i+1].getNumeroFuncionarios()){
        cont2[0] = cont2[0] + 1; 
        if(vet[i].getID() > vet[i+1].getID()){
          cont2[0] = cont2[0] + 1;
          cont2[1] = cont2[1] + 1;  
          swap(i, i+1); 
        }
  ...
  public int[] countingsort(int[] cont2) {
      //Array para contar o numero de ocorrencias de cada elemento
      int[] count = new int[getMaior() + 1];
      Municipio[] ordenado = new Municipio[n];

      for(int i = 0; i < n; i++){
        ordenado[i] = new Municipio();
      }

      //Agora, o count[i] contem o numero de elemento iguais a i
      for (int i = 0; i < n; count[vet[i].getNumeroFuncionarios()]++, i++);
      ...
      //Ordenando
      for(int i = n-1; i >= 0; ordenado[count[vet[i].getNumeroFuncionarios()] - 1] = vet[i].clone(), count[vet[i].getNumeroFuncionarios()]--, i--){
          cont2[0] = cont2[0] + 1;
          cont2[1] = cont2[1] + 1;  
      }

      //Copiando para o array original
      for(int i = 0; i < n; vet[i] = ordenado[i].clone(), i++){
          cont2[0] = cont2[0] + 1;              
      }
