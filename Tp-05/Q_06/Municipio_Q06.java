...
class Municipio_Q06{
  ...
  public static void main (String[]args)throws Exception{
    ...
    lista.shellsort(i,cont2);
    lista.mostrar();
    tempo2 = System.currentTimeMillis();
    tempo = tempo2 - tempo;
    Arq.openWrite("529934_shellsort.txt");
...
class ListaMunicipio{
  ...
  public int[] shellsort(int tam, int[] cont2) {
    int h = 1;
    ...
    do {
      ...
      for(int cor = 0; cor < h; cor++){
        insercaoPorCor(cor, h, tam, cont2);
      }
    }while (h != 1);
  ...
  public int[] insercaoPorCor(int cor, int h, int tam, int[] cont2){
    Municipio tmp;
    int j;
    for (int i = (h + cor); i < tam; i+=h) {
      tmp = vet[i].clone();
      j = i - h;
      while ((j >= 0) && comparaString(vet[j].getID(), tmp.getID(), vet[j].getUF(), tmp.getUF())){
        vet[j + h] = vet[j].clone();
        cont2[0] = cont2[0] + 1;
        cont2[1] = cont2[1] + 1;
        j-=h;
  ...
  public boolean comparaString(int id1, int id2, String str1, String str2){

    for(int i = 0; i < 2; i++){//Copara as strings char a char no decorrer do tamanho da menor delas
      ...
      else if(i == 1){//se todos os caracteres comparados forem iguais, o ID dos municipios é comparado
        if(id1>id2){
          return true;
        }
        else{
