...
class Municipio_Q11{
  ...
  public static void main (String[]args)throws Exception{
    ...
    lista.sort(cont2,cont);
    lista.mostrar();
    tempo2 = System.currentTimeMillis();
    tempo = tempo2 - tempo;
    Arq.openWrite("529934_mergesort.txt");
...
class ListaMunicipio{
  ...
  //private Municipio[] tempMergArr;
  private int n;

  ListaMunicipio(int tamanho){
    vet = new Municipio[tamanho];
    //tempMergArr = new Municipio[tamanho];
  ...
  public int getMaior(Municipio a, Municipio b, int[] cont2){//para comparar dois elementos 
    if(a.getNumeroComissionarios() < b.getNumeroComissionarios()){
      cont2[0] = cont2[0] + 1;
      return 1;
    }
    else if(a.getNumeroComissionarios() == b.getNumeroComissionarios()){
      if(a.getID() < b.getID()){
        cont2[0] = cont2[0] + 1;
        return 2;
      }
  ...
  public int[] sort(int[] cont2, int tamanho){//inicializador do mergesort
    Municipio[] tempMerg = new Municipio[tamanho];
    for(int i = 0; i < tamanho; i++){
      tempMerg[i] = new Municipio();
    }
    mergeSort(vet, tempMerg, 0, n-1, cont2);
    return cont2;
  }
  public int[] mergeSort(Municipio[] vet, Municipio[] tempMerg, int ini, int fim, int[] cont2){
    if(ini < fim){
      int meio = (ini + fim)/ 2;  
      mergeSort(vet, tempMerg, ini, meio, cont2);
      mergeSort(vet, tempMerg, meio + 1, fim, cont2);
      intercalar(vet, tempMerg, ini, meio, fim, cont2);
    }
    return cont2;
  }

  public int[] intercalar(Municipio[] vet, Municipio[] tempMerg, int ini, int meio, int fim, int[] cont2){
    for(int k = ini; k <= fim; k++){
      tempMerg[k] = vet[k].clone();
      cont2[1] = cont2[1] + 1;
    }
    ...
    for(int k = ini; k <= fim; k++){
      if(i > meio){
        vet[k] = tempMerg [j].clone();
        cont2[1] = cont2[1] + 1;
        j++;
      }
      else if(j > fim){
        vet[k] = tempMerg[i].clone();
        cont2[1] = cont2[1] + 1;
        i++;
      }
      else if(getMaior(tempMerg[i], tempMerg[j], cont2) == 1 || getMaior(tempMerg[i], tempMerg[j], cont2) == 2){
        if(getMaior(tempMerg[i], tempMerg[j], cont2) == 1){
          cont2[0] = cont2[0] + 1;
        }
        else{
          cont2[0] = cont2[0] +2;
