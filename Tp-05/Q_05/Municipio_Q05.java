...
class Municipio_Q05{
  ...
  public static void main (String[]args)throws Exception{
    ...
    lista.insercao(i,cont2);
    lista.mostrar();
    tempo2 = System.currentTimeMillis();
    tempo = tempo2 - tempo;
    Arq.openWrite("529934_insercao.txt");
...
class ListaMunicipio{
  ...
  public int[] insercao(int tam, int[] cont2) {
    Municipio tmp;
    int j;
    for (int i = 1; i < tam; i++) {
      tmp = vet[i].clone();
      j = i - 1;
      while ((j >= 0) && comparaString(vet[j].getID(), tmp.getID(), vet[j].getNome(), tmp.getNome())){
        vet[j + 1] = vet[j].clone();
        cont2[0] = cont2[0] + 1;
        cont2[1] = cont2[1] + 1;
        j--;
  ...
  public boolean comparaString(int id1, int id2, String str1, String str2){
    ...
    String[] temporaria1;
    String[] temporaria2;
    temporaria1 = str1.split(" ");
    temporaria2 = str2.split(" ");
    ...
    for(int i = 0; i < aux; i++){//Copara as strings char a char no decorrer do tamanho da menor delas
      ...
      else if(i == aux-1){//se todos os caracteres comparados forem iguais, o ID dos municipios é comparado
        if(id1>id2){
          return true;
        }
        else{
