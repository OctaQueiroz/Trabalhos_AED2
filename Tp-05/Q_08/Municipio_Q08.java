...
public void quicksort(int esq, int dir, int[] cont2) {
        int i = esq, j = dir;
        Municipio pivo = vet[(dir+esq)/2].clone();
        while (i <= j) {
            while (vet[i].getPopulacao() < pivo.getPopulacao()){
              i++;
              cont2[0] = cont2[0] + 1;
            }  
            while (vet[j].getPopulacao() > pivo.getPopulacao()){
              j--;
              cont2[0] = cont2[0] + 1;
            }  
            if (i <= j) {
        ...
        if (esq < j)  quicksort(esq, j, cont2);
        if (i < dir)  quicksort(i, dir, cont2);
    }
